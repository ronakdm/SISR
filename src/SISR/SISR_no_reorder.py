...
def multi_ica(
    ...
    for k in tqdm(range(max_iter)):
        ...
        # ── ablation: periodic W-row reordering disabled ──────────────────
        # (original ica.py reorders W's rows by model-behavior correlation
        # every 1000 iterations here; commented out for this ablation so the
        # rows stay in whatever order the gradient updates put them in)
