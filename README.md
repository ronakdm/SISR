# MultiICA
Stochastic algorithms for multi-trial, supervised independent component analysis.
...
| ----------- | ----------- |
| `src`   | Source code, in which the full Multi-ICA algorithm is implemented via the `multi_ica` function in `ica.py`. |
| `notebooks`   | Visualizations such as training curves and principal component embeddings for the micro-ECoG Reach dataset (`figure_reach.ipynb`) and the simulated datasets (`figure_baselines.ipynb`, `figure_nonconvex.ipynb`). |
